...
/**
 * Edit the profile fields a user owns. Email and role are deliberately not
 * editable here — changing either is an account operation, not a profile one.
 */
export function updateProfile(userId, patch = {}) {
  const user = findUserById(userId);
  if (!user) return { ok: false, reason: "not_found" };

  const name = patch.name === undefined ? user.name : String(patch.name).trim();
  if (!name) return { ok: false, reason: "name_required" };
  ...
  if (patch.division !== undefined) user.division = patch.division;
