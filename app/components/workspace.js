...
import { ErrorState, OfflineBar, Toast, useOnline } from "./ui.js";
...
import { Records, Family } from "./patient/records.js";
import { Settings } from "./settings.js";
import { JoinAsDoctor, DoctorPending } from "./join-as-doctor.js";
import { Icon } from "./icons.js";
import { DoctorWorkspace, useDoctorSelf } from "./doctor-workspace.js";
...
export function Workspace({ portal = "public" }) {
  /**
   * Which portal this is, and therefore whose session it will act on.
   *
   * The cookie is one cookie for the whole origin, so an admin who has signed
   * in at /admin still has a session when they open /. The public portal
   * declines to run as them anyway: its job is the patient and doctor
   * application, and quietly switching a visitor into an admin workspace
   * because of a cookie set on another page is not something the person asked
   * for. Each portal handles the roles it is for and ignores the rest.
   */
  const admin = portal === "admin";
  const portalRoles = admin ? ["admin"] : ["patient", "doctor"];
  ...
  const boot = useCallback(async () => {
    ...
    const me = signedIn && portalRoles.includes(signedIn.role) ? signedIn : null;
    setUser(me);
    setActive(me ? HOME[me.role] : "dashboard");
    // There is no marketing page for administration — it opens at the sign-in.
    setScreen(me ? "app" : admin ? "auth" : "landing");
    if (!me) setLoading(false);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [refreshReference, admin]);
  ...
  if (screen === "auth") {
    return (
      <AuthPage
        ...
        reference={reference}
        api={api}
        onBack={admin ? null : () => setScreen("landing")}
